...
data_files = ["T_0.3_m4", "T_0.3_m5", "T_0.3_m6", "T_0.3_m7"]
Ms = ["4", "5", "6", "7"]
...
for idx, file_name in enumerate(data_files):
    ...
    try:
        ...
        # Subplot styling
        axs[idx].set_title(r"$N_{\rm M} ="+ Ms[idx]+"$", fontsize=15)
        # axs[idx].grid(True, linestyle="--", alpha=0.6)
        if idx==0:
            axs[idx].legend(fontsize=11, loc=(0.5,0.5))
        else :
...
plt.savefig("chi_evolution_vs_M.png", dpi=300)
plt.savefig("chi_evolution_vs_M.pdf")
