...
channel_labels = [f'$\gamma_{i+1}$' for i in range(max(len(d) for d in data))]
...
for idx, ax in enumerate(axes):
    ...
    # Create bar plot
    ax.bar(labels, percentages, color='skyblue', edgecolor='black')
    
    # Add percentage labels on top of bars
    for i, pct in enumerate(percentages):
        ax.text(i, pct + 0.5, f'{pct:.1f}%', ha='center', va='bottom', fontsize=13)
    
    # Set title and labels
    # ax.set_title(f'Channel Distribution (Set {idx+1})')
    ax.set_title(r"$k_{\rm B}T ="+ temperatures[idx]+"\Gamma$", fontsize=15)
