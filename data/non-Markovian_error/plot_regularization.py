...
data_files = ["T_0.1_m6_R1","T_0.1"]
regularization_modes = ["1", "2"]
...
for idx, file_name in enumerate(data_files):
    ...
    try:
        if idx==0:
            ...
            # Subplot styling
            axs[idx].set_title(r"${\rm regularization\  mode} ="+ regularization_modes[idx]+"$", fontsize=19)
            # axs[idx].grid(True, linestyle="--", alpha=0.6)
            axs[idx].legend(fontsize=15, loc="best")
            axs[idx].tick_params(axis="both", labelsize=12)
            if data[-1, 3]/data[-1, 0] > 20 :
                axs[idx].set_yscale("log")
        else:
            t = data[:, 0]
            data_1 = np.loadtxt(file_name)
            data = data-data_1
            ...
            # Subplot styling
            # axs[idx].set_title(r"$regularization mode ="+ regularization_modes[idx]+"$", fontsize=15)
            # axs[idx].grid(True, linestyle="--", alpha=0.6)
            axs[idx].legend(fontsize=15, loc="best")
            axs[idx].tick_params(axis="both", labelsize=12)
            # if data[-1, 3]/data[-1, 0] > 20 :
...
for ax in axs:
    ax.set_xlabel(r'$t$ ($1/\Gamma$)', fontsize=19)
for ax in [axs[0]]:
    ax.set_ylabel(r"$[\chi_m]_1$ ($1/\Gamma$)", fontsize=19)

for ax in [axs[1]]:
    ax.set_ylabel(r"$\Delta\chi_m$ ($1/\Gamma$)", fontsize=19)

# Adjust layout to prevent overlap
plt.tight_layout()

# Save and show the plot
plt.savefig("dchi_evolution_vs_R.png", dpi=300)
plt.savefig("dchi_evolution_vs_R.pdf")
